...
const SELF_GUARDED = new Set([  "/api/fractera/graph-search",
  ...
  // 🔒 ДВЕРЬ ПРИЁМА «СКАЗАТЬ» С ФАЙЛАМИ (200-5) — `benchGuard`; в неё проводит договор `/v1/remember` с формой. Имя стоит до
  // первой сборки: перехваченная привратником, она ответила бы страницей входа, и договор отдал бы её наружу.
  "/api/fractera/remember-ingest",
  ...
  // 🔒 ТРЕТЬЯ ДВЕРЬ МАСТЕРСКОЙ — РЕШЕНИЕ ПО ПОЖЕЛАНИЮ (218-21): утвердить · отклонить · убрать одно.
  // Она тоже под `benchGuard`, и имя стоит здесь по закону 161: не названная привратнику дверь
  // отвечает ПЕРЕАДРЕСАЦИЕЙ на страницу входа, а не ошибкой, и снаружи это неотличимо от её ответа.
  // ✗ Оплачено в тот же день: с секретом машины дверь давала `307`, и прибор получал «Method Not
  // Allowed» вместо JSON — у человека с кукой она при этом работала, то есть дефект был невидим
  // ровно тому, кто проверял руками.
  "/api/fractera/evolution",
  // 🔒 ДВЕРЬ КАНАЛА УПРАВЛЕНИЯ (221): тот же `benchGuard`, имя стоит здесь по закону 161 — не названная
  // привратнику дверь отвечает переадресацией на вход, и снаружи это неотличимо от её ответа.
  "/api/fractera/channel",
]);
...
const PUBLIC_LANG_ROOTS = new Set([
  "/ru",
  "/en",
  // 🪦 ЗДЕСЬ ДВУМЯ СТРОКАМИ БЫЛ ОТКРЫТ ПУБЛИЧНЫЙ АДРЕС ПАСПОРТА (208-3). Документ удалён решением
  // владельца, страница снята — открывать нечего. Оставленная строка открывала бы `404` всему миру.
