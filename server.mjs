...
/**
 * Что говорится строителю сверх того, что он прочитает в папке.
 *
 * 🛑 БЕЗ ЭТОЙ СТРОКИ ОН СТАНЕТ НЕ ТЕМ АГЕНТОМ, И ЭТО ИЗМЕРЕННЫЙ ЗАКОН, А НЕ
 * ОПАСЕНИЕ: рабочая папка есть личность агента, а `CLAUDE.md` этой папки —
 * инструкция АГЕНТА ПАМЯТИ, живущего внутри продукта. Она говорит «ты память,
 * делаешь ровно два дела: записать и извлечь». Строитель, прочитав её как своё
 * описание, откажется строить и начнёт отвечать на вопросы о фактах.
 * ✗ В этом проекте инструкция папки уже перебивала системный промпт — измерено
 * 2026-09-10 на разборе фразы, и разбор пришлось увести в пустую папку.
 */
const BUILDER_PROMPT =
  "Ты строитель ЭТОГО продукта, а не агент, который внутри него живёт. " +
  "Файл CLAUDE.md в рабочей папке описывает агента памяти — того, кем управляет этот продукт; " +
  "это НЕ твоя личность и не твоё задание, а часть предмета твоей работы. " +
  "Твоё дело — менять код, документы и настройки этого дерева по просьбе человека. " +
  "Работай только внутри рабочей папки: соседние деревья на машине принадлежат чужим службам. " +
  "Каждую законченную правку клади коммитом: откат человека возможен ровно до тех пор, пока есть что откатывать."
...
const MODES = {
  ...
  build: (bin) =>
    `${bin} --settings ${BUILDER_SETTINGS} --append-system-prompt ${JSON.stringify(BUILDER_PROMPT)}\n`,
  "claude-check": () => null,
