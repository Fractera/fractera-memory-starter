...
const TOOLS = [
  ...
  {
    // 🔒 ОПИСАНИЕ — ЭТО И ЕСТЬ «КОГДА ЗВАТЬ». Оно приходит агенту в момент
    // решения, и потому здесь сказано о ПОВОДЕ и о ЦЕНЕ, а не об устройстве.
    description:
      "Связи между людьми, местами и событиями: «кто мог знать», «кто с кем работал», «где это было». " +
      "Зови, когда ответа нет в том, что вернул what_i_already_know: там значения о самом человеке, " +
      "здесь — связи между сущностями. Ходов модели НЕ стоит и отвечает за доли секунды. " +
      "Подробности — навык use-knowledge-graph.",
    inputSchema: {
      properties: {
