...
//
// 🔒 ПРОГОНЫ ПРИБОРА В ЧЕЛОВЕЧЕСКИЙ КОРПУС СЛУЧАЕВ НЕ ПОПАДАЮТ — они помечены
// полем `probe`. Корпус заведён для вопросов человека и его вердиктов; заваленный
// машинными прогонами, он так же бесполезен, как пустой.
//
...
async function askAll(test) {
  ...
  for (const { expect, q } of test.questions) {
    const from = logLines()
    const r = await call("/api/fractera/graph-search", { body: JSON.stringify({ probe: true, question: q }), method: "POST" })
    await settle()
