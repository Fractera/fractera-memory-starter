...
console.log("ВОПРОС                                   | БЛИЖЕ ПОРОГА | ВЕРХНИЙ  | СРЕДИ ВСЕХ | БЛИЗОСТЬ | мс")
let hitsA = 0
let exactA = 0
let amongA = 0
for (const { mark, q } of A) {
  ...
  // 🔒 ТРИ РАЗНЫХ УТВЕРЖДЕНИЯ, И СВОДИТЬ ИХ НЕЛЬЗЯ: «достаточно близко» ·
  // «верхним оказался тот самый» · «нужный кусок ЕСТЬ среди отданных».
  //
  // ✗ ВТОРОЕ И ТРЕТЬЕ Я СПЕРВА СМЕШАЛ, И ЭТО БЫЛА ОШИБКА ИЗМЕРЕНИЯ, А НЕ
  // СТРОГОСТЬ: дверь отдаёт ВСЕ куски ближе порога, и агент видит их все. Мерить
  // только верхний значит мерить не то, чем пользуются. Верхний важен отдельно —
  // он говорит о качестве ранжирования, — но полезность решает третья колонка.
  const exact = Boolean(r.json.found) && String(top?.text ?? "").includes(mark)
  const among = (r.json.near ?? []).some((p) => String(p.text ?? "").includes(mark))
  if (exact) exactA += 1
  if (among) amongA += 1
  console.log(
    `${q.slice(0, 40).padEnd(40)} | ${(r.json.found ? "да" : "нет").padEnd(12)} | ${(exact ? "да" : "НЕТ").padEnd(8)} | ${(among ? "да" : "НЕТ").padEnd(9)} | ${String(score).padEnd(8)} | ${r.json.askMs}`,
  )
  console.log(`   ждали «${mark}», отдано кусков ${(r.json.near ?? []).length} → верхний: ${String(top?.text ?? "—").slice(0, 70)}`)
}
...
// 🔒 ВЕРДИКТ — ПО ТОМУ, ЧТО ПОЛУЧАЕТ АГЕНТ: нужный кусок среди отданных.
// Качество ранжирования печатается рядом числом и в вердикт не входит: верхний
// кусок важен человеку у экрана, а модель читает всё, что пришло.
console.log(`  качество ранжирования: верхним оказался нужный в ${exactA} из ${A.length}`)
say(amongA >= 4, `испытание A: нужный кусок среди отданных в ${amongA} из ${A.length} (планка 4)`)
