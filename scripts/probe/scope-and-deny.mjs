...
// 🔒 УТВЕРЖДЕНИЕ ПЕРЕНАЦЕЛЕНО ШАГОМ 184, И ЭТО НЕ ПОДГОНКА ПОД РЕЗУЛЬТАТ.
// Менялось СОСТОЯНИЕ, а не правило: раньше отрицанию не хватало цикла
// дообучения, теперь — нити, в которой только и есть что опровергать. Правило
// осталось прежним: «не поддержано» обязано называть недостающее.
s.say(
  Boolean(d?.note) && d.note.includes("thread"),
  "«deny»: сказано, ЧЕГО именно не хватает",
  d?.note ?? "—",
)
