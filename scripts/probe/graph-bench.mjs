...
if (work) {
  ...
  //
  // ✗ ИЗМЕРЕНО 2026-09-12, И ЭТО НАСТОЯЩАЯ НАХОДКА ПРИБОРА: движок ОБРЕЗАЕТ имя
  // в отчёте — `bench/probe-189-1789...[1 files]` вместо полного. Первая
  // редакция проверки искала полное имя и краснела на исправной работе.
  // 🔒 ЛЕЧЕНИЕ — СРАВНИВАТЬ ВИДИМУЮ ЧАСТЬ, А НЕ ОСЛАБЛЯТЬ УТВЕРЖДЕНИЕ: обрезок
  // всё ещё содержит нашу метку `probe-189-` и начало отметки времени, то есть
  // чужой посев под него не подойдёт. Меняется точность прибора, а не правило.
  const head = String(work.job ?? "").split("...")[0]
  say(
    head.length > "bench/probe-189-".length && `bench/probe-189-${stamp}`.startsWith(head),
