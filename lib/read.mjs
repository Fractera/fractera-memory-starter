...
/**
 * Ответ из таблицы по выбранным родам — тем, которые модель назвала из уже записанного.
 *
 * 🔒 ОТВЕЧАЕТ КОД, А НЕ ПЕРЕСКАЗ МОДЕЛИ: значения берутся из строк как есть.
 */
export function fromTable({ kinds, known }) {
  const wanted = new Set((Array.isArray(kinds) ? kinds : []).filter(Boolean))
  const hits = known.filter((k) => wanted.has(k.what)).map((k) => ({ ...k, found_by: "table" }))
  // 🪦 ЗДЕСЬ СЧИТАЛИСЬ СУММЫ. Складывать можно было потому, что у признака реестра стояло
  // накопление `sum-able`; реестр отменён целиком решением владельца 2026-09-15
  // (`development-docs/CANCELLED.md`), и вместе с ним ушло знание «это складывают».
  // 🛑 ПОТЕРЯ НАЗВАНА, А НЕ СПРЯТАНА: «сколько я потратил» теперь возвращает значения по одному,
  // а не их сумму. Возвращать сумму, не зная, что род складываемый, значило бы складывать всё
  // подряд — включая даты и номера.
  return { hits, sums: [] }
}
