...
- это ФРАЗА, а не слово: не "language", а "language_he_speaks_with_us"; не "city", а "city_where_he_lives";
- одно и то же понятие обязано давать одно и то же имя при повторе.
...
export async function remember({ text, who }) {
  ...
  const answer = await ask(EXTRACT, text)
  ...
  for (const f of facts) {
    ...
    noted.push({ became: value, claim: claim || null, was: was ?? null, what: kind })
  }
  ...
  return {
    ...
    what_happened: said || "записывать было нечего",
  }
}
...
export async function recall({ text, who }) {
  ...
  if (hit) {
    return { known: [hit], ok: true, used_model: false, what_happened: `${hit.what.replace(/_/g, " ")}: ${hit.value}` }
  }
