...
import { DIRECTION, insertMessage, MESSAGES, STATUS } from "./messages.mjs"
...
export async function remember(input = {}) {
  ...
  const scopeOne = (extra.scope ?? [])[0] ?? null
  const row = await insertMessage({
    ...
    scope_source: scopeOne ? "said" : null,
...
export async function recall(input = {}) {
  ...
  const answer = async (body) => {
    ...
    return {
      ...
      text: recallText({ known: body.known, lang, objects, what_happened: body.what_happened }),
    }
  }
  ...
  // 🪦 Здесь создавалась корневая таблица человека — её больше нет (206-1).
  ...
  const mine = await sql(
    `SELECT id, created_at, kind, summary, title, source, object_id, rag_source, scope_place, scope_at
       FROM ${MESSAGES} WHERE who = ? ORDER BY id DESC LIMIT ${RECENT}`,
    [who],
  )
  if (!mine.ok) {
    step(`таблица сообщений не ответила: ${mine.error}`)
    return answer({ error: mine.error, ok: false, what_happened: say("read-failed", lang) })
  }
  ...
  const near = await searchSaid({ question: text, who })
...
export async function people({ lang } = {}) {
  // 🔒 ЛЮДИ БЕРУТСЯ ИЗ ЕДИНСТВЕННОЙ ТАБЛИЦЫ (206-3): человек существует для памяти ровно тогда,
  // когда от него пришло хотя бы одно сообщение. Прежде их читали из корневой таблицы, и туда
  // попадал каждый, о ком хоть раз спросили, — даже если он не сказал ни слова.
  const r = await sql(
    `SELECT who, MIN(created_at) AS since, COUNT(*) AS n FROM ${MESSAGES}
       WHERE who IS NOT NULL AND who <> '' GROUP BY who ORDER BY MIN(id)`,
  )
  if (!r.ok) return { error: r.error, ok: false, what_happened: say("read-failed", lang) }

  return {
    ok: true,
    people: r.rows.map((x) => ({ messages: Number(x.n), since: x.since, who: x.who })),
