...
import { checkGiven, discover, suggestNearest } from "./features-discover.mjs"
...
import { gateFacts } from "./write-gate.mjs"
...
- "value" — точное, счётное или текущее значение: имя, город, сумма, дата, порода, модель машины.
  Короткое. Именно оно может лечь в таблицу.
- "story" — рассказ, история, обстоятельство, объяснение, план, содержимое присланного файла:
  «служил в полку с 1994 по 1996», «знакомы со школы», «снимает квартиру в тихом районе», «в плане
  перечислены отчётность и поездка». Такое сохраняется целиком в связях, а не строкой таблицы.
- Сомневаешься — ставь "story": история, записанная строкой таблицы, ломает ответы на вопросы.
- У текущего признака (город, язык, пояс, имя) одна фраза даёт ОДНО значение "value"; всё прочее о нём — "story".
- Роды из списка с пометкой [реестр] — предпочтительны. Новый род заводи только для короткого "value".
...
function featuresByKind() {
  ...
  try {
    for (const f of readFeatures()) {
      // 205-2: снятый признак родом подсказки записи не бывает — иначе модель продолжит класть в него значения.
...
export async function remember(input = {}) {
  ...
  const already = new Set(
    Object.keys(knownNow ?? {}).filter((c) => !BOOKKEEPING.has(c) && !isPair(c)),
  )
  // 🔒 РОДЫ РЕЕСТРА ИДУТ В ТОТ ЖЕ СПИСОК, ЧТО И ЗАВЕДЁННЫЕ (201-6), И ЭТО ЧИНИТ РАЗРЫВ МЕЖДУ
  // ЗАПИСЬЮ И ЧТЕНИЕМ. ✗ Оплачено прогоном 201-6: фраза про расходы легла под именем, которое
  // придумала модель (`money_he_spent_on_purchases_by_item`), а чтение искало реестровый род
  // (`spent_on_a_purchase`) — и «сколько я потратил сегодня?» получило «не знаю» при трёх
  // записанных тратах. Писатель и читатель обязаны звать одно и то же одним именем.
  //
  // 🛑 РЕЕСТРОВЫЕ РОДЫ ИДУТ ПЕРВЫМИ, А НЕ-РЕЕСТРОВЫЕ ПОМЕЧАЮТСЯ, И ЭТО ТОЖЕ ОПЛАЧЕНО ПРОГОНОМ.
  // ✗ Пустая таблица `…__money_he_spent_on_purchases_by_item`, оставшаяся от прежнего прогона,
  // стояла в списке наравне с реестровым родом — и модель, которой велено «бери имя буква в букву»,
  // честно брала ЕЁ. Осадок прошлых опытов перетягивал новую запись на старое имя, а чтение искало
  // по реестру и отвечало «не знаю» при трёх записанных тратах.
  const registryKinds = featuresByKind()
  const fromRegistry = []
  for (const [kind, f] of registryKinds) {
    const val = knownNow?.[kind]
    fromRegistry.push(`- ${kind} — ${f.title}${val ? ` (сейчас: ${val})` : " (пока пусто)"} [реестр]`)
  }
  const others = Object.entries(knownNow ?? {})
    .filter(([c]) => !BOOKKEEPING.has(c) && !isPair(c) && !registryKinds.has(c))
    .map(([c, val]) => `- ${c}${val ? ` (сейчас: ${val})` : " (пусто; вне реестра — предпочитай род из реестра выше)"}`)
  const catalogue = [...fromRegistry, ...others].join(String.fromCharCode(10))
...
Пересмотри его: отменяется ВЫВОД, а не факт — основание из прежнего разбора остаётся в силе.`
  ...
  // 205-3: чужой ключ получает подсказку ближайших ключей памяти, а не голое «такого нет».
...
${catalogue || "(пока ничего)"}`,
  ...
  // ── ВОРОТА ЗАПИСИ (201-9): в таблицу — только значение, рассказ — только в граф ─────────
  //
  // 🔒 РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-09-15: «Сначала чинить, потом прогон». Правило §21.1 жило в подсказке, и
  // модель ему не подчинялась; теперь его исполняет код (`lib/write-gate.mjs`), а подсказка лишь
  // помогает модели не попадать в ворота зря.
  // 🛑 ОТСЕЯННОЕ НЕ ТЕРЯЕТСЯ: оно уходит в документ графа строкой «что осталось только здесь» и в
  // ответ полем `kept_in_graph_only` — с причиной у каждого.
  const gate = gateFacts(facts, registryKinds)
  facts = gate.toTable
  const storyOnly = gate.toGraphOnly.map(
    (g) => `${String(g.fact?.value ?? "").slice(0, 160)} (${g.why})`,
  )
  ...
  return withParams({
    ...
    // 🔒 ЧТО ЛЕГЛО ТОЛЬКО В СВЯЗИ И ПОЧЕМУ (201-9): рассказ, длинное значение нового рода, второе
    // значение текущего признака. Без этого поля «записано не всё» неотличимо от «сказано не всё».
...
export async function recall(input = {}) {
  ...
  // 🔒 205-3: ПРИЗНАКИ ПРИНИМАЕТ И ЧТЕНИЕ. Зовущий, знающий, о чём спрашивает («сколько я потратил» → траты), шлёт ключ —
  // и подбор модели пропускается, как у записи. Закон шестой: без признаков — тот же ответ, дольше.
  const meant = await discover(`${text}${usedInput.length ? ` (контекст: ${words.slice(0, 12).join(" ")})` : ""}`, {
    given: p.features,
  })
  refused = meant.rejected ?? []
  if (!meant.ok) {
    // 🛑 ОТКАЗ ПОДБОРА — ЭТО ОТКАЗ, А НЕ «НИЧЕГО НЕ НАЙДЕНО». Выдать пустой ответ значило бы сказать
    // «я не знаю» там, где память не смогла даже спросить.
    await note({
      asked: `${who}: ${text}`,
      method: "recall",
      ms: Date.now() - startedAt,
      trouble: `подбор признаков не состоялся: ${meant.error ?? "?"}`,
    })
    step(`подбор признаков не состоялся: ${meant.error}`)
    return answer({ error: meant.error, ok: false, used_input: usedInput, what_happened: refusalWords(meant.error, lang) })
  }
  const featureKeys = (meant.features ?? []).map((f) => f.key)
  step(
    featureKeys.length
      ? `вопрос о признаках: ${featureKeys.join(", ")}`
      : `ни один признак реестра не подошёл${meant.not_in_list ? `: ${String(meant.not_in_list).slice(0, 120)}` : ""}`,
  )

  // ── ТОЧНОЕ, СЧЁТНОЕ И ТЕКУЩЕЕ — ИЗ ТАБЛИЦЫ ─────────────────────────────────
  if (featureKeys.length) {
    const { hits, sums } = fromTable({ featureKeys, known })
  ...
  // ── ЧЕСТНОЕ «НЕ ЗНАЮ» ──────────────────────────────────────────────────────
  //
  // 🔒 РЕШЕНИЕ §21.5: «ВОТ ВСЁ, ЧТО ИЗВЕСТНО» НА КОНКРЕТНЫЙ ВОПРОС — НЕ ОТВЕТ. Это перекладывание
