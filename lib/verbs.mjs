...
- Новый род заводи только для короткого "value".
...
// владельца 2026-09-15 (`development-docs/CANCELLED.md`): модель сопоставляет фразу с ТЕМИ ТАБЛИЦАМИ
// И КОЛОНКАМИ, КОТОРЫЕ У ЧЕЛОВЕКА УЖЕ ЕСТЬ, а не с отдельным словарём.
...
export async function remember(input = {}) {
  ...
  // 🔒 МОДЕЛЬ СОПОСТАВЛЯЕТ ФРАЗУ С ТЕМ, ЧТО У ЧЕЛОВЕКА УЖЕ ЕСТЬ, И БОЛЬШЕ НИ С ЧЕМ.
  // Слово владельца 2026-09-15: «агент анализирует имеющиеся таблицы базы данных, чтобы сопоставить,
  // необходимо ли провести запись в одной из соответствующих таблиц, либо добавить колонку к этой
  // таблице, либо добавить новую таблицу».
  // 🪦 Здесь же стоял список родов РЕЕСТРА впереди заведённых — реестр отменён целиком
  // (`development-docs/CANCELLED.md`).
  // 🛑 ПИСАТЕЛЬ И ЧИТАТЕЛЬ ОБЯЗАНЫ ЗВАТЬ ОДНО И ТО ЖЕ ОДНИМ ИМЕНЕМ, и теперь это обеспечено самим
  // устройством: оба смотрят в один и тот же список — колонки и таблицы этого человека.
