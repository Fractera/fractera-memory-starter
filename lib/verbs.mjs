 ...
 */
async function toGraph({ anchors = [], history = [], pointers = [], said, who }) {
  const names = [...anchors, String(who ?? "")].map((a) => String(a ?? "").trim()).filter(Boolean)
  const put = await putSaid({ anchors: names, kept_as_history: history, pointers, said, who })
  return put
}
...
export async function remember(input = {}) {
  ...
  const { report: paramReport, values: extra } = readParams(input, [
    "scope", "media", "links", "youtube", "deny", "need_table", "thread", "features",
  ])
...
${catalogue || "(пока ничего)"}`,
    ...
    if (!answer.ok) {
      ...
      const kept = await toGraph({ history: givenRefused, pointers: [], said: text, who })
      await note({
        asked: text,
...
export async function recall(input = {}) {
  ...
  const answer = async (body) => {
    ...
    if (body.ok && typeof text === "string" && text.trim()) {
      ...
      if (found.ok) {
        objects = found.results.map((r) => ({ id: r.id, kind: r.kind, messageId: r.messageId, ok: true, score: r.score, title: r.title }))
        step(`объекты по смыслу: ${objects.length}`)
      } else {
