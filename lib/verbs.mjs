...
export async function remember(input = {}) {
  ...
  // 🪦 ЗДЕСЬ СТОЯЛО СОЗДАНИЕ КОРНЕВОЙ ТАБЛИЦЫ (`ensureRoot`) И ЧТЕНИЕ СТРОКИ ЧЕЛОВЕКА (`rowOf`).
  // Своих таблиц память не заводит (решение владельца 2026-09-15), а `rowOf` при этом ещё и ПИСАЛ:
  // `INSERT OR IGNORE` рождал пустую строку на каждого, кто хоть раз что-то сказал.
  //
...
Пересмотри его: отменяется ВЫВОД, а не факт — основание из прежнего разбора остаётся в силе.`
  ...
  // 🪦 Здесь стояли ворота записи `gateFacts` (201-9), решавшие «колонка или таблица», и проверка
