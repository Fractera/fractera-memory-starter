 ...
 */
export async function remember({ lang, text, who }) {
  ...
  if (!who || !text) {
    ...
    return { error: "need-who-and-text", ok: false, what_happened: say("need-who-and-text", lang) }
  }
  if (!canThink()) {
    await note({
      asked: text,
    ...
    return refusal(REFUSAL.NO_CLI, lang)
  }
...
${catalogue || "(пока ничего)"}`,
  ...
  if (!answer.ok) {
    ...
    return refusal(answer.refusal, lang)
  }
  ...
  if (facts.length === 0) {
    ...
    return { noted: [], ok: true, what_happened: say("no-facts-in-phrase", lang) }
  }
  ...
  if (!columns.ok) {
    return { error: columns.error, ok: false, what_happened: say("columns-unreadable", lang) }
  }
 ...
 * просто позже».
 */
export async function recall({ lang, text, who }) {
  const startedAt = Date.now()
  if (!who) {
    await note({ method: "recall", ms: 0, trouble: say("need-who", "ru") })
    return { error: "need-who", ok: false, what_happened: say("need-who", lang) }
  }
  ...
  if (!row) return { known: [], ok: true, what_happened: say("nothing-known", lang) }
