...
export async function note(entry) {
  try {
    const { answer, asked, decisions, dropped, method, ms, model, returned, skipped, trouble } = entry ?? {}

    let block = ""
    block += `## ${stamp()} · ${method ?? "?"}${ms === undefined ? "" : ` · ${ms} мс`}${NL}${NL}`
    // 🔒 ИМЯ ОТВЕТА — ЯКОРЬ, ПО КОТОРОМУ ЗАПИСЬ НАХОДЯТ ЧЕРЕЗ ТРИ ДНЯ (218-24).
    // 🎯 Вопрос владельца 2026-09-17: «ты реально думаешь, что после сброса сессии агент по этому
    // маленькому сообщению восстановит всю цепочку и поймёт, почему ответ не понравился?» Нет — и
    // именно поэтому у каждой записи журнала теперь есть имя ответа, а у заявки строителю — досье,
    // собранное по этому имени. Без якоря блок ищется по времени, то есть угадывается.
    if (answer !== undefined && answer !== null) block += `**Ответ:** ${short(answer, 40)}${NL}${NL}`
