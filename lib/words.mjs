...
export const WORDS = {
  en: {
    // ── форма ответа человеку (206-10, требование владельца 2026-09-16) ──────
    "answer-feedback": "Comment on this answer if it does not suit you — your comment is used to teach the model.",
    "answer-found": "Here is what I found:",
    "answer-not-parsed": "What I could not do: parse your words into facts —",
    "answer-unclassified": "I could not classify this record into any category: it does not fit any service used in the project now, and I could not pick a suitable solution among those that could be taken into development — but I did save your request to the database. If you want to clarify what it may be connected to, continue the conversation and I will tell you more.",
    "answer-kept": "Kept in your memory:",
    "answer-object-kept": "The object is kept in your memory's history.",
    "answer-object-lead": "You uploaded {kind}. In short:",
    "answer-understood": "If I understood you correctly, you meant:",
    ...
    "think-thread-unknown": "that thread of reasoning no longer exists: ask again without the identifier, and memory will start a new one",
    ...
    "need-text": "the call was rejected at the door: there is nothing to remember — «text» is required",
    ...
    // 🔒 ЧЕСТНОЕ «НЕ ЗНАЮ» (201-6): «вот всё, что известно» на конкретный вопрос — это не ответ, а
    // перекладывание работы на того, кто спросил, и повод принять чужой факт за свой.
    "dont-know": "I do not know this: nothing about it is written down",
    "known-count": "known: {n}",
  ...
  ru: {
    // ── форма ответа человеку (206-10, требование владельца 2026-09-16) ──────
    "answer-feedback": "Прокомментируйте ответ, если он вас не устраивает, — ваш комментарий пойдёт на обучение модели.",
    "answer-found": "Вот что нашлось:",
    "answer-not-parsed": "Чего я не смог: разобрать сказанное на факты —",
    "answer-unclassified": "Я не смог классифицировать эту запись ни в одну из категорий: эта запись не подходит ни к одному сервису, который используется в проекте сейчас, я не смог подобрать подходящее решение среди тех, которые можно было бы запустить в разработку, но я всё-таки сохранил ваш запрос в базу данных. Если вы хотите уточнить, с чем это может быть связано, продолжите разговор — я расскажу более подробно.",
    "answer-kept": "В память сохранено:",
    "answer-object-kept": "Объект сохранён в истории вашей памяти.",
    "answer-object-lead": "Вы загрузили {kind}. Коротко:",
    "answer-understood": "Если я правильно понял, вы имели в виду:",
    ...
    "think-thread-unknown": "такой нити размышления больше нет: спросите заново без идентификатора — память начнёт новую",
    "think-timed-out": "модель не ответила за отведённое время",
    "think-unreachable": "модель недоступна",

    "need-text": "вызов отвергнут на входе: нечего запоминать — нужен текст",
    ...
    "dont-know": "этого я не знаю: об этом ничего не записано",
