...
/**
 * Глубина от человека (§6 паспорта, решение владельца 2026-09-08).
 *
 ...
 *   ask    — сказать зовущему, что не хватает и почему (спрашивает человека зовущий, не память — паспорт §3).
 *   join   — поискать в связанных сообщениях.
 */
export const ON_MISSING = ["silent", "ask", "join"]

/**
 * От чего зависит истинность значения — охват (паспорт §3): календарь и место. Пустой охват значит «не
...
export function problemsOf(features) {
  ...
  for (const f of features) {
    ...
    if (!DEPTHS.includes(f.depth)) fail(`«${key}»: глубина «${f.depth}» не 0 и не 1 — второй порядок в таблицу не кладут (§6)`)
