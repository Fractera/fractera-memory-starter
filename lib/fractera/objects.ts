...
// §5 прежнего замысла). Один адрес, один секрет, одна дверь, уже проверяющая права.
 ...
 *
 * 🔒 ИЗМЕРЕН НА НАСТОЯЩИХ ФАЙЛАХ ВЛАДЕЛЬЦА (218-16): голос, снимок, PDF, код, видео с главной FES,
 * страница MDN — вопросы своими словами. Верные попадания 0.58 · 0.56 · 0.50 · 0.48 · 0.45 · 0.39;
 * мусор 0.35 · 0.34 · 0.33 · 0.33 · 0.32 · 0.30. При прежних 0.30 на «есть фотография кота» находился
 * баннер с фурами (0.34). Граница — между 0.35 и 0.39; взято 0.36, ближе к мусору: лишнее «не знаю»
 * дешевле уверенной подмены. 🛑 Выборка — двенадцать точек, число уточняется следующим замером.
 */
export const OBJECT_NEAR = 0.36;
...
export async function keep(input: {
  /** 218-16: путь источника — какая служба и её сущность прислали вещь (207-2). */
...
> {
  ...
  const row = (extra: Record<string, unknown>) =>
    insertMessage({
      ...
      // 🔒 218-16: ПУТЬ ИСТОЧНИКА У ВЕЩИ — КАК У ФРАЗЫ. ✗ Живой замер: файлы, присланные с путём
      // `memory/probe/218-16`, легли с `legacy/api` — дверь загрузки путь не передавала, и откуда пришла
      // вещь, память не знала. Закон 207-2: у каждого входящего сообщения есть путь источника.
 ...
 * «Через ingest».
 * 🔒 ОДНА ФУНКЦИЯ НА ВСЕХ, КТО НЕ ДЕЛАЕТ ВТОРОГО ХОДА: вложения «Сказать» и внутренние двери стенда,
...
export async function ingest(input: {
  /** 218-16: путь источника; без него строка вещи получала «legacy/api». */
  source_path?: string | null;
  anchors?: string[];
  author?: string;
  bytes: Uint8Array;
