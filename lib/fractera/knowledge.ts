...
// 🔒 ФАЙЛ ПЕРЕНЕСЁН, А НЕ НАПИСАН ЗАНОВО (189-1, 2026-09-12).
//
// Источник: `fractera-telegrambot-starter/lib/fractera/knowledge.ts`, шаги 161–171.
// Способ работы назван владельцем дословно: «стратегия вырезать вставить
// адаптировать… там в принципе мы добились уже хорошего результата надо его
// только адаптировать и улучшить если мы щас начнём писать этот код с нуля то
// опять напишем очень сырой и бедный вариант».
//
// 🛑 ДВЕ КОПИИ ОДНОГО КОДА РАСХОДЯТСЯ МОЛЧА, И ЭТО НАЗВАНО, А НЕ СПРЯТАНО.
// У чата свой экземпляр остаётся жить: его память работает сегодня. Вопрос
// «кто хозяин» решается тогда, когда чат начнёт звать службу памяти, — и это
// решение владельца, а не правка следующей сессии.
//
// ✗ ЧЕГО ЗДЕСЬ ПОКА НЕТ, ХОТЯ ИЗМЕРЕНО 2026-09-12: `ask()` не шлёт графу
// ключевые слова, и потому граф извлекает их МОДЕЛЬЮ — 3538 мс против 666 мс
// с нашими словами (повтор без слов отвечает за 372 мс, но это его кэш, а не
// отсутствие вызова). Прежний код чата этого тоже не делал. Лечится в 189-4:
// `hl_keywords`/`ll_keywords` приходят от нас. До тех пор чтение графа стоит
// хода модели, и это здесь сказано, а не умолчано.
