...
const common = {
  ...
  // 🔒 207-3: У ОТВЕТА ЕСТЬ ИМЯ, И ИМ ЕГО КОММЕНТИРУЮТ (третий глагол `feedback`). Имя — номер
  // строки записанного сообщения, а не выдуманный ключ: ответ без строки не комментируется, и
  // обещать ему имя нельзя.
  // 🔒 219-4: ИМЯ УКАЗЫВАЕТ НА САМ ОТВЕТ, А НЕ НА ВХОДЯЩЕЕ СООБЩЕНИЕ. До 219 это был номер строки
  // ВОПРОСА — комментарий человека привязывался не к тому, что он прочитал. Старые имена дверь
  // `feedback` по-прежнему принимает: сломать чужих потребителей ради чистоты нельзя.
  answer_id: z
    .string()
    .optional()
    .describe("имя этого ответа — номер его строки в памяти; им ответ комментируется через feedback"),
  // 🔒 207-3: ОБЯЗАТЕЛЬНЫЙ АТРИБУТ ТВЁРДОСТИ — требование владельца 2026-09-16. Утвердительный ·
  // предположительный · зависящий от параметра, и у третьего названо, от какого именно.
  // Пусто значит «сказать нечего», а не «уверенно нет».
  certainty: z.enum(["affirmative", "presumed", "depends"]).optional()
    .describe("ответ утвердительный, предположительный или зависящий от параметра"),
  depends_on: z.array(z.string()).optional().describe("какие параметры надо назвать, чтобы ответ стал утвердительным: place · time"),
}
...
export const RememberOutput = z
  .object({
    ...
    // 🪦 Поле `thread` снято 206-9 вместе с нитью разбора: продолжение прежнего разговора память
    // снаружи не ждёт и наружу не обещает.
  })
  .loose()
...
export const RecallOutput = z
  .object({
    ...
    known: z
      ...
      .optional()
      .describe("что известно: род, значение, claim, basis"),
    ...
    rejected: z
      .array(z.object({ key: z.string(), why: z.string(), what_happened: z.string(), nearest: z.array(z.string()).optional() }).loose())
      .optional()
      .describe("что из присланного не принято: причина отказа"),
  })
  .loose()
