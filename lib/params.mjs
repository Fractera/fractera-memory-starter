 ...
 */
export const MEDIA_KINDS = ["image", "video", "audio", "pdf", "html", "text"]
...
const SPEC = {
   ...
   *
   * 🔒 РОД НАЗЫВАЕТ ЗОВУЩИЙ, А НЕ УГАДЫВАЕТ ПАМЯТЬ ПО РАСШИРЕНИЮ. Расширение
    ...
    check: (v) =>
      ...
      v.every(
        (m) =>
          ...
          (isText(m.url) || isText(m.id)),
      ),
    formNote: `вложение — это {kind, url|id}, где kind один из: ${MEDIA_KINDS.join(" · ")}`,
    note: "вложения приняты и записаны в объектное хранилище; их разбор идёт тем же циклом, что и фраза",
    works: true,
  },
  thread: {
    check: isText,
