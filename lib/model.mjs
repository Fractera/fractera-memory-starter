...
//
// 🔒 ОТКАЗ ПО ДЕНЬГАМ И ОТКАЗ ПО КЛЮЧУ — РАЗНЫЕ ВЕЩИ (замысел
// владельца). Первое чинит владелец платежом, второе —
...
/** Модель расшифровки речи. Образец — служба каналов (`services/channels/server.js:427`). */
const TRANSCRIBE_MODEL = process.env.TRANSCRIBE_MODEL ?? "whisper-1"

/**
 * Расшифровать речь (194-2): байты или путь к файлу → текст.
 *
 * 🔒 ОТКАЗЫ ТЕ ЖЕ, ЧТО У `ask()`, И РАЗЛИЧАЮТ ДЕНЬГИ И ТЕМП: один ключ, один счёт, одни слова для
 * владельца. Второй набор отказов для того же ключа разошёлся бы с первым на первой правке.
 */
export async function transcribe({ bytes, mime = "application/octet-stream", name, path, timestamps = false }) {
  if (!KEY) return { ok: false, refusal: REFUSAL.NO_KEY }
  let data = bytes
  if (!data && path) {
    try {
      ...
      return { ok: false, refusal: REFUSAL.UNREACHABLE, why: `file: ${String(e.message).slice(0, 120)}` }
    }
  }
  const form = new FormData()
  form.append("file", new Blob([data], { type: mime }), name || "audio")
  form.append("model", TRANSCRIBE_MODEL)
  // 🔒 МЕТКИ ВРЕМЕНИ — РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-09-13 («да, добавь метки времени в расшифровку»):
  // сегменты `verbose_json` дают начало и конец каждой реплики. Без флага ответ прежний.
  if (timestamps) {
  ...
  if (res.status === 401 || res.status === 403) return { ok: false, refusal: REFUSAL.WRONG_KEY }
  if (res.status === 429) {
    const body = await res.text().catch(() => "")
    const noMoney = body.includes("insufficient_quota") || body.includes("billing")
    return { ok: false, refusal: noMoney ? REFUSAL.NO_MONEY : REFUSAL.UNREACHABLE, why: "429" }
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "")
    return { ok: false, refusal: REFUSAL.UNREACHABLE, why: `http-${res.status} ${body.slice(0, 160)}` }
  }
  const body = await res.json().catch(() => null)
  if (typeof body?.text !== "string") return { ok: false, refusal: REFUSAL.BAD_ANSWER }
  const segments = Array.isArray(body.segments)
    ? body.segments
        .map((s) => ({ end: Number(s.end) || 0, start: Number(s.start) || 0, text: String(s.text ?? "").trim() }))
        .filter((s) => s.text)
    : []
  return { model: TRANSCRIBE_MODEL, ok: true, segments, text: body.text.trim() }
}
