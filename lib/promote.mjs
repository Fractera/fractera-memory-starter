...
export async function promote({ basis, claim, kind, newValue, who, words }) {
  ...
  // 🔒 ВТОРОГО ЗНАЧЕНИЯ МОЖЕТ И НЕ БЫТЬ, И ЭТО ЗАКОННЫЙ СЛУЧАЙ (183-6).
  // Обычно род переезжает в таблицу ИМЕННО потому, что пришло второе значение;
  // но зовущий вправе потребовать таблицу сразу (`need_table`), и тогда
  // переезжает одно-единственное. Вставить здесь `null` значило бы завести
  // пустую строку — знание, которого никто не говорил.
  let row = movedIn.lastInsertRowid ?? null
  if (newValue !== null && newValue !== undefined && newValue !== "") {
    ...
    // Указатель ведёт на НОВОЕ значение: именно о нём говорит фраза, которая только что пришла.
    row = added.lastInsertRowid ?? row
  }
  ...
  return { added: newValue ?? null, moved: old.v, ok: true, row, table }
}
...
export async function appendToTable({ basis, claim, kind, value, who }) {
  ...
  // 🔒 НОМЕР СТРОКИ ЕДЕТ НАРУЖУ (201-5): на него указывает документ графа. Указатель без номера
  // говорит «где-то в этой таблице» — то есть не отвечает на тот вопрос, ради которого заведён.
  return r.ok ? { added: value, ok: true, row: r.lastInsertRowid ?? null, table } : r
}
