...
// 🪦 ПРЕДЕЛ 20 МБ СНЯТ 2026-09-13 СЛОВОМ ВЛАДЕЛЬЦА: «тут не должно быть никакого лимита по килобайтам… мы
// сохраняем обычный файл, который потом будем просматривать просмотрщиками». Своего предела у памяти нет.
// 🛑 ФИЗИЧЕСКИЕ ПРЕДЕЛЫ ОСТАЛИСЬ И НАЗВАНЫ: nginx `client_max_body_size 200m` и слой данных — 200 МБ на файл;
// расшифровка OpenAI принимает звук до 25 МБ и откажет словами (`model-unreachable`, http-413).

// 🔒 СПИСОК РОДОВ ВЫНЕСЕН В `kinds.mjs` (200-5): его читают и кнопки загрузки стенда, а этот модуль экрану недоступен.
import { EXT } from "./kinds.mjs"
...
- "full": a description detailed enough that another AI could RECONSTRUCT the object from it alone.
  ...
  For source code — THIS IS THE ONE KIND WHERE "full" IS AN ANALYSIS, NOT A RECONSTRUCTION: say what the
  code DOES for a person in plain words first (e.g. "makes a button pulse while a request is running"),
  then what it exports and what it imports and from where, the technologies and libraries it relies on,
  and anything notable (side effects, limits, security). Do NOT copy the source, do NOT walk through it
  line by line, quote at most a few identifiers. Keep "full" under 3000 characters.
  For a web page or video snapshot — THIS IS AN ANALYSIS, NOT A RECONSTRUCTION, the same rule as for source code:
  say what the page, web app or video is FOR and how it is built — its purpose, sections or chapters, the
  interactive elements (forms, fields, buttons — what each does), the media judged by their attributes, and its
  address. Do NOT retell the content, do NOT copy lists of links, sponsors, adverts or navigation, and do NOT
  repeat the chapter list — it is already stored beside you. Keep "full" under 2500 characters. Never claim to
  have seen pixels: you read extracted metadata and attributes.
...
export function checkShape(d, kind = "") {
  ...
  // 🔒 У КОДА ВЕРХНИЙ ПРЕДЕЛ, И ЭТО СЛОВО ВЛАДЕЛЬЦА 2026-09-13: «его задача понять и описать, что этот элемент
  // кода выполняет, не стоит задачи для кода переписать всё его содержимое». Инструкция просит до 3000 знаков;
  // длиннее 5000 — описание переписывает файл, а не объясняет его. Остальные роды предела не имеют: там
  // полное описание обязано позволять реконструкцию.
  if (kind === "code" && d.full.trim().length > 5000) return `code-full-${d.full.trim().length}-chars`
  // 🔒 У СНИМКА СТРАНИЦЫ И РОЛИКА ПРЕДЕЛ ТОЖЕ ЕСТЬ (195-13, слово владельца «только описание и структуру»): его задача — сказать, ЧТО это и
  // как устроено, а не пересказать содержимое. ✗ Оплачено записью 35: описание ролика вышло пересказом спонсоров и ссылок на 4000+ знаков.
  if (kind === "web" && d.full.trim().length > 4000) return `web-full-${d.full.trim().length}-chars`
 ...
 */
export async function describe({ bytes, mime = "", name, kind: forced = "", url = "" }) {
  const started = Date.now()
  if (!bytes?.length) return { ok: false, refusal: REFUSAL.EMPTY }
  // 🔒 РОД `web` НЕ ВЫВОДИТСЯ ИЗ ИМЕНИ (195-2): снимок ссылки — файл `.md`, и по расширению он был бы текстом. Род называет
  // зовущий — дверь стенда ссылок; остальные роды по-прежнему решает расширение.
  const kind = forced === "web" ? "web" : kindOf(name, mime)
  ...
  try {
    ...
    if (kind === "audio") {
      ...
      // 195-2: снимок страницы — то, что браузер извлёк после скриптов. Модель читает снимок, а не страницу: картинок она не видит.
      tools = ["Read"]
      user = `Object kind: web page snapshot — what a real browser extracted from ${url || "an address"} after its scripts ran (visible text, headings, links, buttons, forms, fields, media by attributes). File name "${name}". Read the file ${file} and describe the page.`
    } else if (kind === "code") {
      tools = ["Read"]
    ...
    if (!r.ok) return r
    const bad = checkShape(r.data, kind)
