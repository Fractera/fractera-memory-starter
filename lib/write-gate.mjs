...
//
// 🔒 ЗАКОН, КОТОРЫЙ ЭТИ ВОРОТА ИСПОЛНЯЮТ, — §21.1 ПАСПОРТА: граф получает ВСЁ сказанное; таблица —
...
/**
 * Сколько слов у значения, чтобы считаться ЗНАЧЕНИЕМ, а не рассказом, — для родов ВНЕ реестра.
 *
 ...
 * @param {Map<string, {aggregate: string, key: string}>} registry — роды реестра по имени рода
 * @returns {{toTable: any[], toGraphOnly: Array<{fact: any, why: string}>}}
 *
 * Правила — и у каждого своя оплаченная причина:
 *  ① `shape: "story"` → только граф. История, объяснение, обстоятельства — не значение (§21.1).
 *  ② род ВНЕ реестра → в таблицу только короткое значение (≤ NEW_KIND_MAX_WORDS слов); длиннее —
 *     рассказ под видом рода, и ему место в графе. ✗ `how_long_he_has_known_his_friend_denis`.
 *  ③ род реестра с накоплением `last` → одна фраза даёт ОДНО значение: остальные значения того же рода
 *     из той же фразы уходят в граф. ✗ «Севилья…» и «снимает квартиру в тихом районе…» как два города.
 * 🛑 Ничего не выбрасывается: всё, что не легло в таблицу, возвращается с причиной и ложится в граф.
 */
export function gateFacts(facts, registry) {
  const toTable = []
  const toGraphOnly = []
  ...
  for (const f of Array.isArray(facts) ? facts : []) {
    ...
    const feature = registry.get(kind)
    if (!feature) {
      if (wordsIn(value) > NEW_KIND_MAX_WORDS) {
        toGraphOnly.push({ fact: f, why: `род вне реестра, а значение длиннее ${NEW_KIND_MAX_WORDS} слов — это рассказ` })
        continue
      }
      toTable.push(f)
      continue
    }
    if (feature.aggregate === "last") {
      if (lastSeen.has(kind)) {
        toGraphOnly.push({ fact: f, why: "у текущего значения одна фраза даёт одно значение" })
        continue
      }
      lastSeen.add(kind)
    }
