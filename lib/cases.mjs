...
export async function caseBook(limit = 25, order = "desc") {
  await ensureCases()
  // 🔒 ПОРЯДОК ПО ДАТЕ — НА СЕРВЕРЕ И БЕЛЫМ СПИСКОМ (195-11, слово владельца: «сделай сортировку по дате опционально»). На экране
  // «сначала старые» по последним 25 показало бы старые из свежих, а не самые старые. Направление уходит в текст SQL — параметром
  // его не передать, поэтому допустимых значений ровно два. Сортировка по `id`: номера растут со временем, а время — до секунды.
  const dir = order === "asc" ? "ASC" : "DESC"
  const rows = await sql(
    `SELECT id, store, question, legacy, found, entities, words_ms, ask_ms, model_turn,
            verdict, why, created_at
       FROM ${CASES} ORDER BY id ${dir} LIMIT ?`,
    [Number(limit)],
  )
 ...
 */
/**
 * Удалить ОДИН случай по номеру (195-11, слово владельца: «сделай во всех кнопку удалить»).
 *
 * 🔒 РОВНО ОДИН И ТОЛЬКО ПО НОМЕРУ: массового удаления дверью по-прежнему нет (закон 189-5 — вердикты человека не стираются пачкой).
 * 🔒 НЕСУЩЕСТВУЮЩИЙ НОМЕР — `not-found`, А НЕ ТИХИЙ УСПЕХ: экран не имеет права подтверждать удаление того, чего не было.
 */
export async function forgetCase(id) {
  const n = Number(id)
  if (!Number.isInteger(n) || n <= 0) return { error: "no-case", ok: false }
  await ensureCases()
  const found = await sql(`SELECT id FROM ${CASES} WHERE id = ?`, [n])
  if (!found.ok) return { ok: false, ...found }
  if (!(found.rows ?? []).length) return { error: "not-found", ok: false }
  const r = await sql(`DELETE FROM ${CASES} WHERE id = ?`, [n])
  return r.ok ? { ok: true, removed: 1 } : { ok: false, ...r }
}
