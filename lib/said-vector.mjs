 ...
 *
 * 🔒 ПОРОГ ЭТОГО КОРПУСА ИЗМЕРЕН НА ЭТОМ КОРПУСЕ (218-2, живой блок владельца 2026-09-16), И ЭТО
 * НЕ ТО ЖЕ ЧИСЛО, ЧТО У СКЛАДА. Прежде здесь стояло `0.28` — число склада (`lib/fractera/vectors.ts`,
 * `NEAR`), повторённое «чтобы не разошлось». Разошлось другое: **корпус**. У объектов свой порог
 * (`OBJECT_NEAR`) ровно по той же причине — измеряется не хранилище, а то, что в нём лежит.
 *
 * ✗ ЧТО ПОКАЗАЛ ЗАМЕР. Девять вопросов владельца подряд: на КАЖДЫЙ приходило пять строк, включая
 * вопрос о том, чего память не знала вовсе («что там с наушниками» → чехлы, встреча, служебная
 * строка прибора). Пустого ответа не случилось ни разу — то есть память не умела сказать «не знаю».
 *
 *   верные попадания:  0.457 · 0.501 · 0.557
 *   мусор:             0.281 · 0.309 · 0.318 · 0.322 · 0.337 · 0.362 · 0.376
 *   граница:           между 0.376 (худший мусор) и 0.457 (лучшее верное)
 *
 * 🔒 ВЫБРАНО 0.40 — ВНУТРИ ЭТОЙ ГРАНИЦЫ, БЛИЖЕ К МУСОРУ. Ошибка в сторону молчания стоит одного
 * «не знаю»; ошибка в сторону выдачи стоит уверенного неверного ответа, которого снаружи не видно.
 * 🛑 ВЫБОРКА МАЛА — девять вопросов. Порог уточняется следующим замером, и это сказано здесь, а не
 * подразумевается: число, выданное за окончательное, никто больше не проверит.
 * 🛑 ДАЛЬНЕЕ НЕ ВЫБРАСЫВАЕТСЯ, А ПОМЕЧАЕТСЯ (`far`): у склада нет пустого ответа — есть «самое
 * близкое из того, что лежит». В ответ человеку дальнее не идёт, но в цепочке разбора оно видно.
 */
export const NEAR_SAID = 0.4
