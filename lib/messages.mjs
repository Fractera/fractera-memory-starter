...
// 🔒 С 195-2 РОДОВ ДЕСЯТЬ: `web` — ссылка, открытая настоящим браузером. Слово владельца (паспорт §3): «то что мы сохраняем
...
export const LATE_COLUMNS = [
  ...
  // повтор (паспорт §3 ②: «по умолчанию… ничего не делаем возвращаем то ответ который уже существуют»).
  "url TEXT",
]
 ...
 * 🔒 ПОВТОРНАЯ ССЫЛКА ПО УМОЛЧАНИЮ — «НИЧЕГО НЕ ДЕЛАЕМ, ВОЗВРАЩАЕМ ТО, ЧТО УЖЕ ЕСТЬ» (паспорт §3 ②). Сорвавшаяся попытка
