 ...
 * @property {string} from            путь источника: откуда пришёл вызов (207-8)
 * @property {string} about           имя ответа, который комментируют: ans_<номер> (211-3)
 ...
 *
 * @typedef {"say" | "ask" | "comment" | "raw"} BenchMode
 *
 * 🔒 ЧЕТЫРЁХ ГЛАГОЛОВ НЕТ И НЕ БУДЕТ (211-3, слово владельца: «нужны три глагола и кнопка отправить
 * неразобранный запрос»). "raw" — не глагол, а способ послать тело КАК НАБРАНО, минуя сборку стенда:
 * им проверяют то, чего органы управления ещё не умеют выразить.
 *
...
export const EMPTY_PARAMS = /** @type {BenchParams} */ ({
  ...
  thread: "",
  wantChain: false,
  // 🔒 СТЕНД НАЗЫВАЕТ СЕБЯ ЧЕСТНО (207-8): он часть службы памяти, и его сущность — стенд.
  // 🪦 Здесь стояло «от чьего имени» со служебным именем человека: людей память больше не считает.
  from: "memory/bench",
  // 🔒 ПУСТО, ПОТОМУ ЧТО ИМЯ ОТВЕТА РОЖДАЕТСЯ ТОЛЬКО ПОСЛЕ ОТВЕТА. Стенд подставляет последнее
  // полученное сам — 36 знаков руками не набирают (тот же довод, что был у имени нити).
  about: "",
...
export function buildCall({ lang, mode, params, supported, text }) {
  // 🔒 ГЛАГОЛ ВЫБИРАЕТСЯ РЕЖИМОМ, А НЕ УГАДЫВАЕТСЯ ПО ПОЛЯМ (211-3).
  const method = mode === "say" ? "remember" : mode === "comment" ? "feedback" : "recall"
  ...
  const wanted = [
    ...
    // 🔒 ПУТЬ ИСТОЧНИКА (207-8) ИДЁТ СТРОКОЙ, КАК ЕГО НАБРАЛИ: разбор — работа памяти, и повторять
    // его здесь значило бы завести вторую правду о том, что считается годным именем службы.
    ["from", String(params.from ?? "").trim() || null],
    // 🔒 «О КАКОМ ОТВЕТЕ» ЕДЕТ ТОЛЬКО У КОММЕНТАРИЯ — и договор сам скажет «не доезжает», если его
    // прислать другому глаголу: список поддержанного порождён из договора, а не написан здесь.
    ["about", mode === "comment" ? String(params.about ?? "").trim() || null : null],
    // 🪦 Нить, опровержение, прежний разговор, уже найденное и требование таблицы сняты шагом 206-9:
    // вход памяти — текст и объекты.
