...
export async function think(system, user, options = {}) {
   ...
   * дешевле присланного заново (паспорт §18). Значит вернуться к цепочке —
  ...
  const args = [
    ...
    "--model", MODEL,
    "--output-format", "json",
    // 🔒 ИНСТРУМЕНТЫ И НАВЫКИ ПРИХОДЯТ ИЗ ПАПКИ СЛУЖБЫ — ИЛИ НЕ ПРИХОДЯТ ВОВСЕ.
    //
    // ✗ НАЙДЕНО 2026-09-12 ВОПРОСОМ ВЛАДЕЛЬЦА «откуда модель должна понять,
    // каким инструментом звать граф»: `--strict-mcp-config` БЕЗ `--mcp-config`
    // означает ноль серверов. Пять инструментов памяти были объявлены в
    // `.mcp.json` и **не доезжали до агента ни разу**: он работал текстом на
    // входе и текстом на выходе. Способность, написанная и не подключённая,
    // снаружи неотличима от отсутствующей — закон, уже оплаченный в соседней
    // службе трижды.
    //
    // 🔒 ПУТЬ АБСОЛЮТНЫЙ И СЧИТАЕТСЯ ОТ КОРНЯ СЛУЖБЫ, А НЕ ОТ ПАПКИ РАЗБОРА.
    // Думает агент по-прежнему в пустой папке — там нет чужого `CLAUDE.md`, —
    // но объявление инструментов берётся оттуда, где оно живёт.
