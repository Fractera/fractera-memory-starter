...
| the row | table `messages_that_came_into_memory` | kind, title, summary, tags, source path, source, author, links to the three above, status |
...
|---|---|---|
| from an outside tool or a bot, as bytes | `POST /v1/remember`, multipart: JSON in the `payload` part, the file in a `files` part | memory itself, in the same call |
| as an address (Telegram gives files as URLs) | `POST /v1/remember`, JSON `media: [{ "url": … }]` | memory downloads it, then describes it |
...
Default to **one call to `remember`** — a thing with nothing to say about it goes with an empty `text`. The two-step stand path exists only because a human wants to read and
...
- **`from`** — the source path: which service and which of its entities sent it (`telegram/bot/roma-armstrong`).
  The first segment must be a service of `SERVICES.json`; an unknown one is refused and nothing is written.
...
| `is-a-page` | the address answers `text/html`: a web page is not a file. Send it as a link instead — `remember` with `links` (pages) or `youtube` (videos); see the `use-links` skill |
...
rather than repeating what you sent — the hand `open_object { id }` for the card and text, or
...
- Web pages and YouTube are not files: they go through `links` / `youtube` of `remember` — the page is
  opened by the AI-browser service, the video is read by the official YouTube API (`use-links`).
