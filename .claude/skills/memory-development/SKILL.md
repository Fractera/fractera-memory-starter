...
description: >
  ...
  skills, its design note and its step records. Use this skill whenever any file in this repository is
...
🔒 **Ask him about results, never about file or function names** (a law paid for on 2026-09-15). A technical decision
...
| `development-docs/LAWS.md` · `BLACKBOX-API.md` | laws this service is built by · what memory does today, and what it deliberately does not |
...
🔒 **An idea travels in one order:** discussed → written into the architect's note → the architect approves or
...
3. The three latest results in `completed-steps/`, the architect's note sections your task touches, `LAWS.md`.
...
Full texts live in `LAWS.md` and the architect's note; read them, never copy them here.
...
- **`CLAUDE.md` holds only what is true now** — no tombstones; history belongs to the architect's note.
- **Instructions and skills stay compact:** the result wanted and the tools available; the model
  finds the way.
- **Describe memory whole, as if built; unbuilt parts get an asterisk and a note** (the architect: «if you just write we did not build it, how will you build it?») — and
  only after reading the architect's note and skills, never from recollection.
- **Memory grows by itself; the registry catches up** (the one-table law of step 206): new kinds are allowed, a feature for
