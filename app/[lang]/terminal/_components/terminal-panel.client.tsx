...
export function TerminalPanel({ lang }: { lang: string }) {
  ...
  // 🪦 У ЧАТА КНОПКА ОТКРЫВАЛА ОКНО НАСТРОЙКИ АГЕНТА — ТОКЕН БОТА, КЛЮЧ OpenAI,
  // ПОДПИСКА, ПРИВЯЗКА КАНАЛА. У памяти бота нет, и из всего окна ей нужна одна
  // строка — вход в подписку. Поэтому кнопка входит сразу, а окно, кнопка
  // «Сессия агента» и привязка канала сюда не поехали (180-3).
