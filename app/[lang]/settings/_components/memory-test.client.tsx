...
//
// 🔒 У ЧЁРНОГО ЯЩИКА ТРИ ГЛАГОЛА — «СКАЗАТЬ», «СПРОСИТЬ» И «ПРОКОММЕНТИРОВАТЬ»; СТЕНД ПРОВЕРЯЕТ ИМЕННО ИХ.
...
type Words = {
  ...
  say: string;
  ask: string;
  /** Третий глагол (211-3) и сырой запрос — не глагол, а способ послать тело как набрано. */
  comment: string;
  raw: string;
  commentHint: string;
  rawHint: string;
  ...
  inputTitle: string;
  answerTitle: string;
  /** Машинная часть и человеческий ответ названы РАЗНО (218-4, слово владельца 2026-09-16). */
...
export function MemoryTest({
   ...
   */
  supported: { feedback: readonly string[]; recall: readonly string[]; remember: readonly string[] };
  words: Words;
}) {
  const [mode, setMode] = useState<BenchMode>("say");
  // 🪦 ЗДЕСЬ ЖИЛО ОТДЕЛЬНОЕ ПОЛЕ ДЛЯ ТЕЛА ЗАПРОСА (214-2): человек пишет словами во всех режимах.
  const [text, setText] = useState("");
  const [params, setParams] = useState<BenchParams>(EMPTY_PARAMS);
  // 🪦 207-8: ЗДЕСЬ ЖИЛ СПИСОК ЛЮДЕЙ. Людей память больше не считает — человек один; экран спрашивает
  // у неё каталог ИСТОЧНИКОВ: какая служба и какая её сущность уже писала.
  const [sources, setSources] = useState<string[]>([]);
  const [sourcesLoading, setSourcesLoading] = useState(true);
  const [shots, setShots] = useState<Shot[]>([]);
  const [busy, setBusy] = useState(false);
  // 🔒 НИТЬ ИЗ ПОСЛЕДНЕГО ОТВЕТА ПАМЯТИ ЖИВЁТ ЗДЕСЬ (184-4): 36 знаков руками не набирают.
  // 🔒 211-3: ИМЯ ПОСЛЕДНЕГО ОТВЕТА ЖИВЁТ ЗДЕСЬ — им комментируют, и набирать его руками никто не
  // станет. 🪦 Прежде здесь лежало имя НИТИ разбора: её предмет снят 206-9, а проп продолжал ездить
  // в органы управления и не использовался там ничем — мёртвый проп с живым видом.
    ...
    void (async () => {
      try {
        const r = await fetch("/api/fractera/memory-test", {
          body: JSON.stringify({ get: "sources" }),
          cache: "no-store",
          headers: { "Content-Type": "application/json" },
          method: "POST",
        });
        const data = (await r.json()) as { body?: { sources?: Array<{ path?: string } | string> } };
        const list = (data?.body?.sources ?? [])
  ...
  const preview = buildCall({
    ...
    mode,
    params,
    supported: mode === "say" ? supported.remember : mode === "comment" ? supported.feedback : supported.recall,
    text,
  });
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (preview.files.length) {
        ...
        // 🔒 СЛОВА БЕЗ ГЛАГОЛА УХОДЯТ НА ДВЕРЬ БЕЗ ИМЕНИ ГЛАГОЛА: память относит их сама и
        // говорит первой строкой ответа, как поняла. Тело здесь — те же обычные слова.
        init = {
          body: JSON.stringify({ unsorted: true, body: { lang, text: text.trim() } }),
          cache: "no-store",
          headers: { "Content-Type": "application/json" },
          method: "POST",
        };
      ...
      // 🔒 ИМЯ ОТВЕТА ВЫНИМАЕТСЯ СРАЗУ: им его потом комментируют, и набирать руками его никто не станет.
      // 🪦 Здесь вынималось имя НИТИ — её нет с 206-9, и поле в ответе давно не приходило: стенд ловил
      // то, чего не бывает, и молчал об этом.
      const gotId = (answer?.body as { answer_id?: unknown } | undefined)?.answer_id;
      if (typeof gotId === "string" && gotId) setLastAnswerId(gotId);
      setShots((s) => [
        {
          asked,
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="grid gap-3 md:h-[600px] md:grid-cols-2">
        ...
        <div className="flex max-h-[70vh] min-h-0 flex-col overflow-hidden rounded-md border border-muted-foreground/30 md:max-h-none">
          ...
          <div className="max-h-[70%] shrink-0 space-y-2 overflow-y-auto border-b border-muted-foreground/20 p-3">
            <div className="flex flex-wrap gap-2">
              ...
              {/* 🔒 211-3: ТРЕТИЙ ГЛАГОЛ НА СТЕНДЕ, И ПОРЯДОК ТОТ ЖЕ, ЧТО В ДОГОВОРЕ: сказать ·
                  спросить · прокомментировать. Четвёртый глагол был бы выдумкой — слово владельца:
                  «нужны три глагола и кнопка отправить неразобранный запрос». */}
              {modeButton("comment", words.comment)}
              {modeButton("raw", words.raw)}
            </div>
            <p className="text-[length:var(--fs-small)] text-muted-foreground">
              {mode === "say" ? words.sayHint : mode === "comment" ? words.commentHint : mode === "raw" ? words.rawHint : words.askHint}
            </p>

            {/* 🔒 214-2: ПОЛЕ ОДНО И ТО ЖЕ ВО ВСЕХ РЕЖИМАХ — ЧЕЛОВЕК ПИШЕТ СЛОВАМИ.
                ✗ Здесь стояло поле для JSON: «неразобранный запрос» я прочитал как «тело без
                разбора парсером», а владелец имел в виду «я не говорю, какой это глагол».
                Его слова: «я же не машина, чтобы писать JSON». */}
            <textarea
        ...
        {/* ПРАВАЯ КОЛОНКА — РАЗМЫШЛЕНИЯ ПАМЯТИ: ход мыслей и машинная часть дословно.
            🎯 Слово владельца 2026-09-16: «Это не ответ памяти… это размышления памяти.
            Вкладка "ответ памяти" сделай ниже». Человеческий ответ живёт под этой сеткой. */}
        <div className="flex max-h-[70vh] min-h-0 flex-col overflow-hidden rounded-md border border-muted-foreground/30 md:max-h-none">
          <div className="border-b border-muted-foreground/20 px-3 py-2 text-[length:var(--fs-small)] font-medium">
            {words.thinkingTitle}
          </div>
      ...
      {/* ОТВЕТ ПАМЯТИ — ЧЕЛОВЕЧЕСКИМИ СЛОВАМИ, ПОД РАЗМЫШЛЕНИЯМИ (218-4).
          🔒 ТОЛЬКО ПОСЛЕДНИЙ ОТВЕТ И ТОЛЬКО ТЕКСТ: лента машинных снимков стоит выше, а здесь
          человек читает то, что память ему сказала. Крупнее и шире — это не приписка к JSON,
          это и есть ответ. ✗ Оплачено словом владельца: «набор отрывков, не связанных между
          собой фраз, разделённых квадратными фигурными скобками, я называю иероглифами». */}
      <div className="mt-4 rounded-md border border-muted-foreground/30" data-testid="human-answer">
        <div className="border-b border-muted-foreground/20 px-3 py-2 text-[length:var(--fs-small)] font-medium">
          {words.answerTitle} · {words.answerHuman}
        </div>
        <div className="p-4">
          {(() => {
            const last = shots[0];
            const text = typeof (last?.body as { text?: unknown } | null)?.text === "string"
              ? (last!.body as { text: string }).text
              : "";
            if (!text) {
              return <p className="text-[length:var(--fs-small)] text-muted-foreground">{words.answerNone}</p>;
            }
            return (
              <>
                <div className="mb-2 flex flex-wrap items-baseline gap-x-3 text-[length:var(--fs-small)] text-muted-foreground">
                  <span>{last?.at}</span>
                  <span>
                    {words.took}: {last?.ms} мс
                  </span>
                </div>
