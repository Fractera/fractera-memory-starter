...
// у бота, а не здесь.
//
// 🔒 НАРУЖУ ИДУТ СЛОВА, НОМЕРА УРОВНЕЙ ОСТАЮТСЯ ВНУТРИ (договор §12). Человеку
...
export type BenchControlWords = {
  ...
  from: { label: string; hint: string; loading: string };
  /** Имя ответа, который комментируют (211-3). */
  about: { label: string; hint: string };
...
export function BenchControls({
  ...
  params: BenchParams;
  /** Кто памяти уже писал — спрошено у неё же адресом каталога `GET /v1/sources` (207-8). */
  sources: string[];
  ...
  const set = [
    ...
    params.from !== "memory/bench" && words.from.label,
  ...
  return (
    <div className="rounded-md border border-muted-foreground/30">
      ...
      {/* ① О КАКОМ ОТВЕТЕ КОММЕНТАРИЙ (211-3).
          🔒 Показан только там, где договор его принимает: `has("about")` берёт список из самого
          договора, а не из нашего представления о том, какому глаголу он «положен».
          🔒 Кнопка подставляет имя последнего ответа — 36 знаков руками не набирают. */}
      {has("about") ? (
        <Row hint={words.about.hint} label={words.about.label} ok words={words}>
          <div className="flex gap-2">
            ...
            {lastAnswerId ? (
              <button
                className="shrink-0 rounded-md border border-muted-foreground/30 px-2 text-[length:var(--fs-small)] hover:bg-muted"
                onClick={() => onChange({ about: lastAnswerId })}
                type="button"
              >
                {lastAnswerId}
              </button>
      ...
      {/* ② ОТКУДА ПРИШЁЛ ВЫЗОВ — ПУТЬ ИСТОЧНИКА (207-8).
          🔒 Список — то, что память уже видела; свой путь набирается руками, потому что новая
          служба и её сущности рождаются раньше, чем попадут в этот список. Незнакомую СЛУЖБУ
          память отвергнет сама — вторая проверка здесь была бы второй правдой. */}
      <Row hint={words.from.hint} label={words.from.label} ok={has("from")} words={words}>
        <input
          className={boxClass}
          list="bench-sources"
          onChange={(e) => onChange({ from: e.target.value })}
          placeholder="memory/bench"
          value={params.from}
        />
        <datalist id="bench-sources">
          {sources.map((p) => (
            <option key={p} value={p} />
          ))}
        </datalist>
        {sourcesLoading ? (
          <p className="text-[length:var(--fs-small)] text-muted-foreground">{words.from.loading}</p>
        ) : null}
      </Row>

      {/* ③ ИСТОРИЯ РАЗГОВОРА — ПЕРЕКЛЮЧАТЕЛЬ И ПОЛЕ.
          🔒 Стоит ВЫШЕ основного поля в раскладке стенда — так в договоре §12:
      ...
      {/* ④ ПРЕЖНИЙ ПОИСК — ДВЕ РАЗНЫЕ ВЕЩИ В ОДНОМ ОРГАНЕ, И РАЗНИЦА НАЗВАНА.
          ...
          (0.0289 → 0.0065 → 0.0036), договор §10.
      ...
      {/* ⑤ ЦЕПОЧКА РАЗМЫШЛЕНИЙ — ДА ИЛИ НЕТ.
          🔒 Умолчание «нет» (договор §4): переполнять свой контекст или нет
