  ...
  -H "x-memory-key: $MEMORY_KEY" \\
    ...
    "scope": [
      ...
      { "place": "London" }
        ...
        {/* 🔒 ПРИМЕРЫ ВЕЩЕЙ (207-5): отдельного глагола для них больше нет — вещь кладётся «Сказать»
            вложением, и текст при этом может быть пустым. */}
        <P>{w.examples.keepFile}</P>
        <Code>{`curl -s ${base}/v1/remember \\
  -H "x-memory-key: $MEMORY_KEY" \\
  -F 'payload={"from":["telegram","bot"]}' \\
  -F "files=@lease.pdf"`}</Code>
        <P>{w.examples.keepUrl}</P>
        <Code>{`curl -s ${base}/v1/remember \\
  -H "Content-Type: application/json" \\
  -H "x-memory-key: $MEMORY_KEY" \\
  -d '{ "media": [{ "url": "https://example.com/note.oga" }], "from": ["telegram","bot"] }'`}</Code>
        <P>{w.examples.find}</P>
        <Code>{`curl -s ${base}/v1/recall \\
  -H "Content-Type: application/json" \\
  -H "x-memory-key: $MEMORY_KEY" \\
  -d '{ "text": "the lease agreement for the flat" }'`}</Code>
        <P>{w.examples.file}</P>
