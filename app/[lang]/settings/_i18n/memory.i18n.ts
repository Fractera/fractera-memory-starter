...
export type MemoryUi = {
  ...
  testBench: {
    ...
    // 🪦 206-5: слов вкладки оценки больше нет — ни лида, ни корпуса случаев. Прогоны стендов
    // с вердиктом человека сняты целиком словом владельца 2026-09-16 «remove old tests».
  ...
  memoryTest: {
    ...
    say: string;
    ask: string;
    /** Третий глагол стенда (211-3): отзыв о названном ответе. */
    comment: string;
    /** Не глагол, а способ послать тело КАК НАБРАНО, минуя сборку стенда. */
    raw: string;
    sayHint: string;
    askHint: string;
    commentHint: string;
    ...
    answerTitle: string;
    /**
     * Заголовок машинной части и заголовок человеческого ответа — РАЗНЫЕ (218-4).
     *
     * 🎯 Слово владельца 2026-09-16: «Это не ответ памяти, замени это название для таблицы, это
     * размышления памяти. Вкладка "ответ памяти" сделай ниже».
     * 🔒 Название лжёт дороже отсутствующего: человек, читающий ход мыслей под шапкой «Ответ
     * памяти», решает, что память так и отвечает — скобками и служебными полями.
     */
...
const EN: MemoryUi = {
  ...
  memoryTest: {
    ...
    thinkingTitle: "Memory's reasoning — the machine part",
    answerHuman: "The answer in plain words",
    answerNone: "Nothing asked yet — the answer will appear here, in plain words.",
    ask: "Ask",
    ...
    controls: {
      ...
      about: {
        hint: "The name of the answer this comment is about: it comes back in answer_id. The button next to it fills in the last one you got.",
        label: "Which answer",
      },
      from: {
        hint: "Which service and which of its entities is calling. The list is what memory has already seen; a new path is typed by hand. An unknown SERVICE is refused by memory itself.",
        label: "Where the call comes from",
        loading: "asking memory who has written to it…",
      },
    },
    ...
    comment: "Comment",
    commentHint: "The comment goes to feedback and names the answer it is about — the third kind of request, not politeness. The name of the last answer is filled in for you.",
    raw: "Without a verb",
    rawHint: "Write in plain words and do not say which verb it is — memory decides: something to remember, a question, or a comment on a previous answer. The first line of the answer says how it read you, so you can correct it.",
    rawTitle: "In your own words",
    say: "Say",
...
const RU: MemoryUi = {
  ...
  memoryTest: {
    ...
    thinkingTitle: "Размышления памяти — машинная часть",
    answerHuman: "Ответ человеческими словами",
    answerNone: "Пока ничего не спрашивали — ответ появится здесь, обычными словами.",
    ask: "Спросить",
    ...
    controls: {
      ...
      about: {
        hint: "Имя ответа, о котором комментарий: оно приходит в поле answer_id. Кнопка рядом подставляет последний полученный.",
        label: "О каком ответе",
      },
      from: {
        hint: "Какая служба и какая её сущность зовёт. Список — то, что память уже видела; свой путь набирается руками. Незнакомую СЛУЖБУ память отвергает сама.",
        label: "Откуда пришёл вызов",
        loading: "спрашиваем память, кто ей писал…",
      },
    },
    ...
    lead: "Три глагола — «Сказать», «Спросить» и «Прокомментировать», у каждого свои расширенные параметры. Каждый вызов идёт в публичный API памяти /v1/* — тем же путём, что у любой программы; адрес, заголовки и тело ниже — ровно то, что уходит.",
    nothingSent: "Пока ничего не отправляли.",
    nothingYet: "Память ещё не отвечала — отправьте фразу слева.",
    comment: "Прокомментировать",
    commentHint: "Комментарий уходит в feedback и называет ответ, о котором он: третий род запроса, а не вежливость. Имя последнего ответа подставляется само.",
    raw: "Без глагола",
    rawHint: "Пишите обычными словами и не говорите, какой это глагол — память решит сама: запомнить, спросить или это отзыв о прошлом ответе. Первой строкой ответа она скажет, как вас поняла, — и это можно поправить.",
    rawTitle: "Своими словами",
    say: "Сказать",
