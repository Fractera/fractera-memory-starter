...
const RU: ApiDocWords = {
  ...
  param: {
    ...
    media: "Вложения: список {url} — адреса файлов. Каждый файл память скачивает и кладёт тем же путём, что keep_object: описание, объектное хранилище, карточка поиска, граф, строка таблицы. Род определяет сама память. Веб-страница (text/html) отвергается кодом is-a-page: ссылки и ролики YouTube память кладёт своим слоем ссылок, и наружу через договор он пока не выведен — метода для ссылок здесь нет. Судьба каждого вложения — в objects.",
