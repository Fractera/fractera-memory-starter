...
const EN: ApiDocWords = {
  ...
  catalogue: {
    ...
    lead:
      "Read-only endpoints answer the question «what does memory have». Memory keeps one table of incoming messages, so there is nothing to ask it about storage names.",
  },
  catalogueItem: {
  ...
  method: {
    // 🪦 207-5 и 207-6: отсюда убраны слова о снятых глаголах — приёме вещи, поиске вещей,
    // открытии вещи, списке людей и двух глаголах о журнале. Список методов страница порождает
    // из договора; слова о том, чего в договоре нет, обещали бы способность, которой нет.
    recall: {
      about:
      ...
      returns:
        "known — what is known; every value carries claim (said by the person or inferred) and basis (what it was inferred from). not_yet_known — what is missing and why it matters; used_model — whether the model was called. depth_asked and depth_used — the limit you asked for and the level memory actually reached. used_input — which of the context you sent took part in the search. chain — the steps of the search, and only if you asked for them. params — the fate of every optional parameter.",
    },
    remember: {
      about:
      ...
      returns:
        "what_happened — words you can say to a person; noted — what exactly was written down, each entry with its claim. On a correction it says out loud that the correction was taken, and names the kind of the value. The previous value is no longer named: it used to live in a column, and since 206-1 a fact is an event of the graph. params — the fate of every optional parameter. objects — the fate of every media attachment: the object id and messageId, or a refusal with its reason.",
    },
  },
  methods: {
    body: "body",
...
const RU: ApiDocWords = {
  ...
  catalogue: {
    ...
    lead:
      "Адреса только на чтение отвечают на вопрос «что у памяти есть». Память ведёт одну таблицу входящих сообщений — спрашивать её об именах хранилищ стало не о чем.",
  },
  catalogueItem: {
  ...
  method: {
    ...
    remember: {
      ...
      returns:
        "what_happened — словами, которые можно произнести человеку; noted — что именно записано, с родом утверждения у каждой записи. При поправке сказано вслух, что она принята, и назван род значения. Прежнее значение больше не называется: оно жило колонкой, а с 206-1 факт стал событием графа. params — судьба каждого необязательного параметра. objects — судьба каждого вложения media: id объекта и messageId либо отказ с причиной.",
    },
  },
  methods: {
    body: "тело",
  ...
  overview: {
    ...
    twoVerbs:
      "Три глагола — и в этом вся мысль. remember меняет то, что память знает; recall не меняет ничего. Всё остальное — каталог, журнал — существует, чтобы объяснить, что память делала, а не чтобы дать дотянуться внутрь.",
  },
  param: {
