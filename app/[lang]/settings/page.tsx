...
import { MemoryTest } from "./_components/memory-test.client";
...
// 🔒 211-3: ТРИ ГЛАГОЛА, И СПИСОК ПОДДЕРЖАННОГО ПОРОЖДАЕТСЯ ИЗ ДОГОВОРА ДЛЯ КАЖДОГО.
function supportedParams(): { feedback: string[]; recall: string[]; remember: string[] } {
  const of = (name: string) =>
    (METHODS.find((m) => m.name === name)?.params ?? []).map((p) => p.name);
  return { feedback: of("feedback"), recall: of("recall"), remember: of("remember") };
}
...
async function MemoryPageBody({
  ...
  // 🪦 ЗДЕСЬ ЧИТАЛСЯ ПАСПОРТ СЛУЖБЫ И СТРОИЛИСЬ ЕГО ВКЛАДКИ (208-3). Документ удалён решением
  // владельца; то, что он объяснял, живёт в инструкции службы и на её главной странице.

  // 🔒 ТРИ СТРАНИЦЫ СТЕНДА — ТА ЖЕ ПОЛОСА ВКЛАДОК, ЧТО У ДОКУМЕНТА (189-1).
        ...
        >
          <div className="space-y-6">
            ...
            {active === "memory-test" && openTab !== "skill" && (
              // 🪦 206-6: под стендом стоял раздел со списком того, что память
              // построила, — то есть со списком специализированных таблиц. Он ушёл вместе
              // с предметом, и связующий островок с ним: тот существовал только ради «отправили →
              // таблицы перечитались».
              <MemoryTest
            ...
            {isTestSection(active) && (
              <section className="space-y-4">
                ...
                <p className="max-w-3xl text-[length:var(--fs-body)] text-muted-foreground">
                  {/* 🪦 ЛИД ВКЛАДКИ «ОЦЕНКА» СНЯТ ВМЕСТЕ С НЕЙ (206-5, «remove old tests»). */}
                  {openTab === "skill"
