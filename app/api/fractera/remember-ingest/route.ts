...
//
// 🎯 СЛОВО ВЛАДЕЛЬЦА 2026-09-14: у чёрного ящика три глагола (с 207-4), «всё остальное это включение в эти два типа»;
...
export async function POST(request: Request) {
  ...
  // 218-16: без `who` строка получала автора «undefined» — пустое имя становится архитектором, как у глаголов
  const who = typeof payload.who === "string" && payload.who.trim() ? payload.who.trim() : "architect"
  // 🔒 218-16: ПУТЬ ИСТОЧНИКА ПРОВЕРЯЕТСЯ ДО ЗАПИСИ ФАЙЛОВ. Незнакомая служба — отказ, и не пишется ничего:
  // иначе файлы легли бы с непроверенным путём раньше, чем глагол его отверг.
  let sourcePath: string | null = null
  if (payload.from !== undefined) {
    const parsed = parseSource(payload.from) as { ok: boolean; text?: string; why?: string; error?: string }
    if (!parsed.ok) return refuse(400, { error: parsed.error ?? "bad-source", what_happened: parsed.why ?? "bad source" })
    sourcePath = parsed.text ?? null
  }
  ...
  const text = rememberText({
    ...
    // 216-2: дверь формы называет отказ так же, как сам глагол — иначе он исчезает на одном из двух входов.
    trouble: (answer.refusal ? answer.what_happened : null) as never,
