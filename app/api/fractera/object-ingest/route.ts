// @api приём объекта целиком: память сама описывает файл и кладёт его в четыре хранилища — внутренняя дверь «Сказать» с вложением
...
// приходит во вложениях «Сказать» (`/v1/remember` формой) с ключом памяти, и `server.mjs` проводит тело сюда потоком по петле
